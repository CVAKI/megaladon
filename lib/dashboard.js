...
// Accessible via /dashboard command inside Megaladon.
...
function startDashboard(port, cfgRef, memRef, messagesRef, statusRef, version) {
  ...
  dashboardServer = http.createServer((req, res) => {
    ...
    if (url.startsWith('/api/memory/')&&req.method==='DELETE') {
      ...
      const mem2=loadMemory();
      // FIX: "Clear All Memory" (seg === 'all') is an explicit, intentional
      // wipe, same as the terminal's /memory clear command — it must bypass
      // saveMemory()'s merge-safe logic (force:true), otherwise the empty
      // facts array gets unioned right back with whatever's on disk (e.g.
      // from a concurrent Megaladon terminal instance) and the "clear"
      // silently does nothing. Deleting a single fact by index (seg is a
      // number) is NOT force — that's a normal edit and should still merge
      // safely with concurrent writes.
      if (seg==='all') {
        mem2.facts=[]; mem2.sessionSummaries=[];
        saveMemory(mem2, { force: true });
      } else {
        const i=parseInt(seg); if (!isNaN(i)) mem2.facts.splice(i,1);
        saveMemory(mem2);
      }
      if (memRef) { memRef.facts=mem2.facts; }
