...
//   command/connections.js  /connection /wa /tm
//   command/tools.js        /searchengine /search /email
...
const { handleSearchEngine, handleSearch, handleEmail } = require('./command/tools');
...
async function dispatchCommand(text, ctx) {
  ...
  if (text === '/tm' || text.startsWith('/tm '))                return handleConnection('/connection teams' + text.slice(3), ctx);

  // ── Web search & email tools ──────────────────────────────────────────────
  if (text.startsWith('/searchengine'))                        return handleSearchEngine(text, ctx);
  if (text.startsWith('/search '))                             return handleSearch(text, ctx);
  if (text.startsWith('/email'))                                return handleEmail(text, ctx);
