...
async function handleProvider(ctx) {
  ...
  console.log('  ' + kp('[4]') + ' NVIDIA NIM'         + (ctx.providerKey === 'nvidia'     ? kp(' ◀') : ''));
  const olOk = await ollamaAvailable();
  console.log('  ' + kp('[5]') + ' Ollama' + (ctx.providerKey === 'ollama' ? kp(' ◀') : '') + ' ' + (olOk ? kp('● running') : rf('○ not running — will auto-install if selected')));
  const ch = await ctx.ask(cr('\n  ❯ ') + ab('Switch [1-5] or Enter to cancel: '));
  const pk = { 1: 'anthropic', 2: 'openrouter', 3: 'groq', 4: 'nvidia', 5: 'ollama' }[ch.trim()];
  if (pk) {
    if (pk === 'ollama' && !olOk) {
    ...
    console.log('\n  ' + kp('✔ Switched. Restart megaladon to apply.'));
  }
  ctx.prompt(); return true;
}
...
async function handleToken(text, ctx) {
  ...
  if (sub === '-set-usage' || sub === '-set-token') {
    ...
    console.log('  ' + dm('Applied to all AI calls. Persisted to ~/.meg/config.json'));
    ctx.prompt(); return true;
  }
