...
async function handleHelp(ctx) {
  console.log('');
  console.log('  ' + cr(C.bold + 'Megaladon v' + VERSION) + C.reset + '  ' + ab(ctx.prov.name + ' · ' + (ctx.modelMeta.label || ctx.modelMeta.id)));
  const sections = {
    'CHAT & MODES': [
      ['/help',           'This help'],
    ...
    'CONNECTIONS': [
      ...
      ['/wa owner <number>',       'Change the owner number Megaladon responds to'],
      ['/wa --reset',              'Wipe WhatsApp session & credentials — forces fresh QR scan'],
    ],
    'SESSION': [
      ['/save [name]',  'Save conversation to ~/.meg/sessions/'],
...
async function handleExport(ctx) {
  ...
  const bubblesHtml = exportMsgs.map(m => {
    ...
    const name      = isUser ? 'You' : '🦈 Megaladon';
...
function handleHistory(ctx) {
  // FIX: was require('../config') / require('../colors') — session.js lives
  // at lib/main/command/session.js, two levels below lib/, same as every
  // other import at the top of this file (which correctly use '../../config'
  // and '../../colors'). The single '../' here resolved to a nonexistent
  // lib/main/config.js, throwing "Cannot find module '../config'" the moment
  // /history was run — this is what surfaced once /history actually started
  // getting used (via the Ctrl+C session-summary fix), not something the fix
  // itself introduced.
  ...
  console.log('');
  // FIX: was missing — every other command handler in this file ends with
  // ctx.prompt() to redraw the "┌[time]────[Megaladon]...──►" banner before
  // the next input. Without it here, the app was still perfectly functional
  // and still listening for input, but the prompt line just never reappeared
  // — looked like it hung, and whatever got typed next (e.g. a stray "1")
  // went straight to the AI as a normal chat message with no visible prompt
  // in front of it.
  ctx.prompt();
  return true;
}