...
const { runToolWithAutoInstall, runAbilities } = require('./toolRunner');
...
async function handleAiMessage(text, ctx) {
  ...
  try {
    ...
    // FIX: was a fully silent `catch (_) {}` — if recordTokens/loadCache
    // ever actually threw (bad permissions, corrupt cache file, etc.), it
    // would fail forever with zero visibility, which is indistinguishable
    // from "Tokens: 0 because nothing happened". Log it instead so a real
    // failure here is actually diagnosable, while still never letting cache
    // bookkeeping break the main chat flow.
    console.log('  ' + require('../../colors').rf('⚠ session-cache token tracking failed: ' + e.message));
  }
  ...
  if (needsToolFollowup) {
    ...
    for (const q of directives.search) {
      ...
      try {
        ...
        // FIX: search never counted toward Progress: X done · Y failed —
        // see recordAction() in session-cache.js for why.
        try {
          const cacheS = sessionCache.loadCache();
          sessionCache.recordAction(cacheS, 'search', q, !(r && r.error));
        } catch (_) {}
      } catch (e) {
        toolResults.push('Search for "' + q + '" failed: ' + e.message);
        try {
          const cacheS = sessionCache.loadCache();
          sessionCache.recordAction(cacheS, 'search', q, false);
        } catch (_) {}
      }
    }
  ...
  // ── Phase 5d: Desktop abilities — real mouse/keyboard/screen control ──────
  // Multi-turn loop, mirroring the tools follow-up above: execute
  // @@ABILITY: directives for real (skills/abilities), feed the actual
  // result back to the model as a user message, let it decide the next
  // step, repeat. Bounded by MAX_ABILITY_TURNS since these tasks are often
  // multi-step (look → click → type → look again) rather than one-shot
  // like search/email. Runs after the tools block so it always operates on
  // the freshest `directives` (tools reparse `reply` above if they ran).
  // Was 6 — too low for real browser-driving tasks (open url → look → click
  // search bar → type → enter → look → click result → look → right-click →
  // look for context menu → click "save as" → look → type filename → enter
  // easily runs 12-15 turns). Bumped to 20 with the safety cap left in place.
  const MAX_ABILITY_TURNS = 20;
  // Whether a turn made real progress is defined by whether any INTERACTIVE
  // action ran (mouse.*/keyboard.*) — NOT by whether every action matched a
  // specific whitelist. That distinction matters: a turn can include a
  // hallucinated/unknown ability name (e.g. the model inventing
  // "@@ABILITY:directives", which isn't real) alongside a real look-around
  // action — checking "every action is in this whitelist" would incorrectly
  // treat that as NOT qualifying for a nudge, when really nothing useful
  // happened at all. Checking "no interactive action ran" catches that case
  // correctly regardless of what unknown/failed action names show up.
  function isInteractiveAction(action) {
    return action.startsWith('mouse.') || action.startsWith('keyboard.');
  }
  if (directives.abilities.length > 0) {
    tracker.start(6, 'Desktop abilities');
    let abilityTurns = 0;
    let nudgedOnce = false; // only nudge once per request — avoid nagging a model that's genuinely stuck

    while (directives.abilities.length > 0 && abilityTurns < MAX_ABILITY_TURNS) {
      abilityTurns++;
      const abilitiesRanThisTurn = directives.abilities; // capture before this iteration's reply overwrites `directives`
      tracker.sub(6, 'run', 'turn ' + abilityTurns + ': ' + directives.abilities.map(a => a.action).join(', '));

      let outcome;
      try {
        outcome = await runToolWithAutoInstall(runAbilities, directives.abilities);
      } catch (e) {
        tracker.sub(6, 'fail', 'ability execution error: ' + e.message.slice(0, 100));
        try {
          const cacheAb = sessionCache.loadCache();
          sessionCache.recordAction(cacheAb, 'ability', directives.abilities.map(a => a.action).join(', '), false);
        } catch (_) {}
        break;
      }

      if (!outcome.ran) {
        tracker.sub(6, 'fail', outcome.error || 'abilities skill not available — see skills/abilities/README.md');
        break;
      }

      outcome.results.forEach(r => {
        if (r.ok) tracker.sub(6, 'pass', r.ability + ' ✔ ' + JSON.stringify(r.result).slice(0, 70));
        else      tracker.sub(6, 'fail', r.ability + ' ✘ ' + r.error);
      });

      try {
        const cacheAb2 = sessionCache.loadCache();
        sessionCache.recordAction(cacheAb2, 'ability', directives.abilities.map(a => a.action).join(', '), outcome.ok !== false);
      } catch (_) {}

      // Feed the real result back — same pattern as tool results above —
      // so the model sees "click landed at x,y" / a fresh screen
      // description and decides its next @@ABILITY: step (or stops).
      ctx.messages.push({ role: 'user', content: outcome.text });

      let followUp;
      try {
        followUp = await callProvider(ctx, [
          { role: 'system', content: systemPrompt },
          ...ctx.messages,
        ]);
      } catch (e) {
        tracker.sub(6, 'fail', 'follow-up call after ability failed: ' + e.message.slice(0, 100));
        break;
      }
      ...
      // ── One-time corrective nudge ─────────────────────────────────────────
      // Only look-around actions ran this turn, and the model emitted no
      // further @@ABILITY: directives — it may have just described the
      // screen and stopped instead of continuing toward the actual task.
      // Give it exactly one chance to self-correct with an explicit prompt
      // before accepting that as the real final answer.
      const onlyLookedAround = abilitiesRanThisTurn.length > 0 &&
          !abilitiesRanThisTurn.some(a => isInteractiveAction(a.action));
      if (!nudgedOnce && onlyLookedAround && directives.abilities.length === 0 && abilityTurns < MAX_ABILITY_TURNS) {
        nudgedOnce = true;
        tracker.sub(6, 'info', 'model only looked around and stopped — sending one corrective nudge');
        ctx.messages.push({
          role: 'user',
          content: 'You only opened something or described the screen just now, but did not perform ' +
              'any click/type/press action — even though the original request likely asked for more ' +
              'than that. If the task is not actually finished, continue right now with the next real ' +
              '@@ABILITY: directive(s) using the LITERAL @@ABILITY:action / {json} / @@END format — ' +
              'not prose, not JavaScript/code syntax. Pick whichever applies: if you just opened a ' +
              'search engine homepage, its search box is already focused, so go straight to ' +
              'keyboard.type then keyboard.press "enter" — no click needed. Otherwise, if you need to ' +
              'click something, use screen.describe first to get real coordinates from its ELEMENT ' +
              'list, then mouse.click. If the task is genuinely finished, or you truly need ' +
              'information only the person can give you, say so plainly instead of a generic "what ' +
              'would you like me to do next?"',
        });
        try {
          const nudgeReply = await callProvider(ctx, [
            { role: 'system', content: systemPrompt },
            ...ctx.messages,
          ]);
          ctx.messages.push({ role: 'assistant', content: nudgeReply });
          reply = nudgeReply;
          ctx.lastReply = reply;
          directives = parseDirectives(reply);
        } catch (e) {
          tracker.sub(6, 'fail', 'nudge follow-up failed: ' + e.message.slice(0, 100));
        }
      }
    }

    if (abilityTurns >= MAX_ABILITY_TURNS && directives.abilities.length > 0) {
      tracker.sub(6, 'info', 'stopped after ' + MAX_ABILITY_TURNS + ' ability turns (safety cap) — remaining @@ABILITY: directives were not run');
    } else {
      tracker.sub(6, 'pass', 'no further @@ABILITY: directives — task complete');
    }

    tracker.done(6, abilityTurns + ' turn(s)');
  }
  ...
  if (directives.emailSend.length === 0 && hadForcedEmailIntent) {
    // FIX: was unconditionally building the canned "couldn't find reliable
    // current information" filler here — discarding the model's actual
    // reply even when that reply was a perfectly good, coherent drafted
    // email (just not in @@EMAIL_SEND: or To:/Subject: format, so neither
    // fallback above caught it). Prefer the model's own prose as the body
    // whenever it looks usable; only fall back to the generic filler when
    // the reply itself is empty or a bare refusal/meta statement.
    const bodyFromReply = looksLikeUsableEmailProse(reply) ? reply.trim() : null;

    directives.emailSend.push({
      to: hadForcedEmailIntent.recipient,
      subject: fallbackSubject(hadForcedEmailIntent.topic),
      body: bodyFromReply ||
          `Hi,\n\nYou asked me to look into "${hadForcedEmailIntent.topic}" — I searched but couldn't find reliable current information on this.\n\n— Sent via Megaladon`,
    });
    console.log('  ' + ab(bodyFromReply
        ? '⟳ using model\'s prose reply as the email body (no @@EMAIL_SEND: / To:-Subject: format found)'
        : '⟳ model did not draft an email — built a fallback "no info found" email instead'));
  }
    ...
    for (const mail of directives.emailSend) {
      ...
      if (confirm === 'y' || confirm === 'yes') {
        try {
          ...
          // FIX: same gap as search — a successful send never counted
          // toward Progress: X done · Y failed. See recordAction() in
          // session-cache.js.
          try {
            const cacheE = sessionCache.loadCache();
            sessionCache.recordAction(cacheE, 'email_send', 'to ' + mail.to, true);
          } catch (_) {}
        } catch (e) {
          console.log('  ' + rf('✘ Send failed: ' + e.message));
          try {
            const cacheE = sessionCache.loadCache();
            sessionCache.recordAction(cacheE, 'email_send', 'to ' + mail.to, false);
          } catch (_) {}
        }
      } else {
