...
// Used by the last-resort email fallback in handleAiMessage.js: when the
// model doesn't emit @@EMAIL_SEND: AND doesn't write a To:/Subject:-shaped
// draft (so parseEmailDraftFallback() above finds nothing either), its raw
// reply text is often still a perfectly good, coherent drafted email — it
// just wasn't in either expected format. Rather than discard that and send
// a generic "couldn't find info" filler instead, the reply itself should be
// used as the body whenever it actually looks like a drafted message.
// Deliberately permissive — the bar is "not obviously unusable", not "looks
// exactly like an email" — since malformed-but-real drafts (missing a
// sign-off, no greeting, etc.) are still far better as a body than the
// generic filler. Rejects only near-empty replies and bare meta/refusal
// statements that clearly aren't a drafted message at all.
function looksLikeUsableEmailProse(text) {
  const t = (text || '').trim();
  if (t.length < 20) return false;
  if (/^(i cannot|i can't|i'm unable|i am unable|as an ai|i don't have access|i do not have access)/i.test(t)) return false;
  return true;
}

// ─── fallbackSubject ──────────────────────────────────────────────────────────
// The forced-email-intent "topic" (from detectEmailComposeIntent above) is
// extracted straight from the person's raw sentence and is meant for feeding
// back to the model as context, not for use as a literal subject line — e.g.
// "to ask if the meeting is still happening" reads as a sentence fragment,
// not a subject. This is only used by the last-resort fallback (when the
// model itself never supplied a subject at all), so it's a light cleanup,
// not a rewrite: drop a leading "to " (common since "to <verb>..." is how
// Case A topics often start) and capitalize, rather than trying to be clever.
function fallbackSubject(topic) {
  let s = (topic || '').trim().replace(/^to\s+/i, '');
  s = s ? s.charAt(0).toUpperCase() + s.slice(1) : '';
  return s || 'Following up';
}
