...
// Wrapped in try/catch: if skills/abilities isn't installed (deps missing,
// folder not present, etc.) the rest of the system prompt must still build
// normally — @@FILE/@@RUN/@@MEMORY/@@SEARCH/@@EMAIL always work regardless.
let ABILITIES_PROMPT = '';
try {
  ({ ABILITIES_PROMPT } = require('../../../skills/abilities'));
} catch (_) { /* abilities skill not installed — prompt block stays empty */ }
...
function buildSystemPrompt(ctx) {
  ...
  const directiveBlock = [
    ...
    '12. Only use @@SEARCH / @@EMAIL_READ / @@EMAIL_SEND / @@ABILITY when the person\'s message',
    '    actually calls for web info, email, or real desktop control — never on your own initiative.',
