...
function parseDirectives(text) {
  ...
  while ((m = searchRe.exec(text)) !== null) results.search.push(m[1].trim());
...
function buildSystemPrompt(ctx) {
  ...
  const directiveBlock = [
    ...
    '@@MEMORY: key=value',
    '',
    'To search the web (only when the person actually asks about current info / facts you\'re unsure of):',
  ...
  if (needsToolFollowup) {
    ...
    for (const spec of directives.emailRead) {
      ...
      try {
        ...
        const items = await listInbox({ limit: n, unreadOnly });
