...
// This file lives at lib/main/aiHandler.js — two levels below the Megaladon
// install root. See runToolWithAutoInstall below.
...
// inside handleAiMessage() below (Phase 6 / 5b / 5c).
...
// follow-up above). When that happens, parseDirectives() finds nothing,
// Phase 5c never runs, and the person just sees a draft with no indication
// that nothing was actually queued to send. This catches that prose pattern
// deterministically and turns it into a real emailSend directive so the
// normal confirm-before-send flow still runs.
//
...
async function handleAiMessage(text, ctx) {
  ...
  // new update" instead of the whole sentence. The extracted recipient is
  // stashed on ctx so Phase 5b's follow-up prompt can explicitly steer the
  // model toward @@EMAIL_SEND: instead of leaving it to infer that itself.
  if (directives.search.length === 0 &&
    ...
    ctx._forcedEmailIntent = emailIntent || null;
  ...
  if (directives.emailSend.length > 0) {
    for (const mail of directives.emailSend) {
      ...
      // (rule 13 below) is what actually instructs it to prefer memory when
