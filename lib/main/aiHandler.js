...
const { autoExtractFacts }                       = require('./factExtract');
...
async function handleAiMessage(text, ctx) {
  ...
  try {
    ...
    if (autoFacts.length) {
      const newOnes = autoFacts.filter(f =>
  ...
  if (directives.search.length === 0 &&
    ...
    directives.search.push(text.trim());
    tracker.note(5, '⟳ forced search fallback — model did not emit @@SEARCH: for a search-worthy message');
  }
  ...
  if (needsToolFollowup) {
    ...
    for (const spec of directives.emailRead) {
      ...
      try {
        const unreadOnly = /unread/i.test(spec);
        const n = unreadOnly ? 10 : (parseInt(spec, 10) || 10);
        const items = await runToolWithAutoInstall(listInbox, { limit: n, unreadOnly });
  ...
  if (directives.emailSend.length > 0) {
    for (const mail of directives.emailSend) {
      ...
      if (confirm === 'y' || confirm === 'yes') {
        try {
          await runToolWithAutoInstall(sendEmail, mail);
