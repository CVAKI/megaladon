...
// FIX: saveConfig used to be a blind full-file overwrite — it wrote out
// whatever THIS process's in-memory `cfg` object held, with no regard for
// what's currently on disk. Any handler that does loadConfig() → mutate one
// field → saveConfig(cfg) is working off its own snapshot. If that snapshot
// was taken before another part of the app (or another process) wrote a
// different field — e.g. /email setup saving cfg.email — saving the older
// snapshot later silently erases that field with no warning. This is the
// same class of bug saveMemory() below was already fixed for; saveConfig
// never got the equivalent fix, which is why email config (and potentially
// mode/model/provider/token settings) could vanish between commands.
//
// Fix: read-merge-write instead of blind-overwrite. Whatever is on disk is
// merged with this process's cfg; this process's values win per-key on
// conflict (it's the most "current" action), but a key this process never
// touched is never silently dropped just because this process's in-memory
// snapshot didn't happen to have it.
//
// deleteKeys lets a caller explicitly remove a top-level key (e.g.
// /email reset clearing cfg.email) — a plain `delete cfg.email` before
// calling saveConfig would NOT survive the merge, since the merge would
// just pull `email` back in from disk. Use deleteKeys for any real,
// intentional removal.
function saveConfig(cfg, opts) {
  const deleteKeys = (opts && opts.deleteKeys) || [];
  try {
    let onDisk = {};
    try {
      if (fs.existsSync(CONFIG_PATH)) onDisk = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'));
    } catch (_) { onDisk = {}; }

    const merged = { ...onDisk, ...cfg };
