...
// ─── recordAction ─────────────────────────────────────────────────────────────
// FIX: tasksDone/tasksFailed (shown as "Progress: X done · Y failed" in
// /memory and in the SESSION STATE block fed back to the model) were only
// ever incremented by recordFile() and recordCommand() — meaning @@FILE:
// writes and @@RUN: commands. Real work done through @@SEARCH:, @@EMAIL_SEND:,
// and @@EMAIL_READ: never touched these counters at all, so any session that
// was entirely emails/searches (no file writes, no shell commands) would
// permanently show "0 done · 0 failed" no matter how much actually happened.
// This is a generic counter for exactly those non-file/non-command actions,
// so email sends and searches finally count as "done" too.
function recordAction(cache, type, detail, success = true) {
  if (success) cache.tasksDone++; else cache.tasksFailed++;
  _addHistory(cache, success ? (type + '_ok') : (type + '_failed'), detail);
  saveCache(cache);
}
...
module.exports = {
  ...
  recordFile, recordCommand, recordError, recordTokens, recordAction,
  buildSessionContext,
  CACHE_PATH,
};